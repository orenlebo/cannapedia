...
המקורות שסופקו לך מגיעים מתאריכים שונים. חובה עליך להשוות אותם באופן אקטיבי.
אם מקור ישן מציג תיאוריה, מיתוס, או טענה — ומקור חדש יותר מפריך, מעדכן, או משנה את המידע הזה (למשל מחקר חדש יותר) — חובה עליך לשקף את ההתפתחות הזו בטקסט.
...
- בשדה "sources" כלול אך ורק כתבות מארכיון מגזין קנאביס שסופקו לך ב-context
- חובה להשתמש ב-URL המדויק שמופיע בשורת "Exact URL:" בכל בלוק מקור. אסור בהחלט לנחש, לבנות, או ליצור URLs בעצמך
- אסור לכלול קישורי Cannabiz או קישורי מסחר אלקטרוני בשדה sources. שדה ה-sources מיועד אך ורק לכתבות רפואיות/מדעיות
...
8. אל תכלול sections על מוצרים מסחריים — רק תוכן מדעי/רפואי`;
  ...
  if (ragData) {
    prompt += `\n\nלהלן מקורות מארכיון מגזין קנאביס ישראלי. השתמש בהם כבסיס עובדתי.
זכור: מידע חדש יותר גובר על ישן (Lex Posterior).
כלול בשדה "sources" את הכתבות שהשפיעו על התוכן שיצרת — חובה להשתמש ב-Exact URL שמופיע בכל בלוק מקור.

${ragData}`;
  } else {
    prompt += `\n\nלא נמצאו מקורות מקומיים. הסתמך על הידע המדעי הגלובלי שלך בלבד.
